...
- 모든 하드웨어의 제품·포인트별 검사 영역(ROI) 지정과 영상 중앙 가이드
...
1. **제품 레시피 → 제품 선택 → 포인트 편집**에서 하드웨어를 선택합니다.
2. **검사 영역 설정 · 영상 확인**을 누르고 해당 카메라 영상에서 사각형을 드래그하거나 **중앙 50%**를 선택합니다.
3. 홀 전체와 측정 여백이 영역 안에 들어오도록 로봇 위치를 맞춥니다. 십자선은 영상 중앙이며 수직 자세를 자동 검증하지 않습니다.
4. **영역 선택 완료 → 포인트 저장 → 초안 저장 → 검증 후 적용**으로 Inspect에 저장·적용합니다.
5. 레시피 목록·생산 검사·PLC 디버그의 **검사 영역 · 위치 확인**으로 가이드를 확인하고 1회 검사합니다.

ROI는 제품 레시피의 `roi: {x,y,width,height}`에 영상 대비 0~1 비율로 저장합니다. 왜곡 보정 사용 시 보정 후 영상이 기준이고,
카메라 YAML의 센서 촬영 범위와 별개입니다. 영상 여백을 제외한 실제 표시 영역에서 좌표를 계산합니다.
**전체 프레임**으로 해제하며 기존 ROI 없는 포인트도 전체 프레임으로 표시합니다. 복제는 ROI를 유지하고 검사 하드웨어를 바꾸면 ROI를 해제합니다.
너트 홀 ROI의 기대 개수는 1개로 고정합니다. 다른 하드웨어는 해당 영역 안의 기대 개수를 입력합니다.
검출은 전체 영상에서 수행하지만 ROI 밖 대상은 판정에서 제외하고, 경계에 걸친 대상이나 홀 측정 여백 부족은 NG입니다.
카메라 배정·촬영 크롭·왜곡 보정을 바꾸면 ROI를 다시 확인하세요. 촬영 이력은 사용한 ROI를 보존하고 결과 오버레이에 영역을 표시합니다.
ROI를 지원하는 Inspect와 함께 배포해야 합니다. 카메라 영상 크기나 연결이 확인되지 않으면 영역 편집을 진행할 수 없습니다.
...
**카메라 설정:** 검사 관리의 첫 번째 탭에서 현재 검색된 Basler 카메라만 표시합니다. 연결되지 않은 카메라는 숨기고 저장값은 보존합니다.
새 장치를 연결하면 재검색으로 표시되며, **저장 및 적용** 성공 시 시리얼별 YAML 항목이 추가됩니다.
**촬영 설정**에서 해상도·센서 ROI·픽셀 형식·노출·게인·화이트밸런스·FPS·타임아웃·트리거·장치 ID 확인값을 편집합니다.
입력 항목·자료형·선택값과 하드웨어 라벨은 Inspect API에서 받아 구성합니다. 시리얼·현재 IP는 읽기 전용 식별값입니다.
각 카메라에 미사용 또는 하드웨어 하나를 배정하고 **보정 파일 선택**으로 최대 64 KiB의 ROS camera_info YAML을 업로드합니다.
촬영 옵션과 보정 파일 모두 시리얼에 연결되어 하드웨어를 바꿔도 유지됩니다. **보정 파일 해제**로 파일 연결을 제거할 수 있습니다. 왜곡 보정 사용 여부를 선택하고 촬영 완료·PLC 연결 해제 후 저장합니다.
Inspect가 카메라를 열어 프레임을 읽고 보정 파일·촬영 해상도를 검증한 뒤 재시작 없이 적용합니다. 저장 후 **영상 확인**으로 확인합니다.
중복 배정·저장 도중 연결 누락·잘못된 옵션·파일은 오류로 표시합니다. 미연결 카메라는 저장 요청에서 제외하며 기존 배정은 유지합니다. 미배정 하드웨어의 촬영 요청은 오류이며 카메라를 임의 대체하지 않습니다.
ROI 정보가 없는 보정 파일은 화면에 표시된 촬영 크기·센서 ROI와 보정 당시 조건이 같은지 확인해야 합니다.
`settings_fields`와 카메라별 `settings`를 제공하는 GET/PUT `/api/production/camera-setup`이 필요하므로 Inspect와 Studio를 함께 업데이트합니다.
설정은 Inspect의 `state/production/cameras/settings.yaml`, 보정 파일은 같은 디렉터리에 저장됩니다.
처음에는 `fastener.yaml`을 읽고, 저장 후에는 카메라 전용 YAML을 적용합니다. 이전 설정 JSON은 기존 슬롯의 촬영 옵션을 포함해 한 번 변환한 뒤 제거합니다.
선택 항목을 비우면 그 장치 옵션은 변경하지 않습니다. 노출·게인 수동값은 자동 모드가 Off일 때 적용됩니다.
하드웨어 트리거 모드는 저장 시 프레임 확인을 위해 외부 트리거가 필요하며 입력이 없으면 저장 오류가 납니다.
장치의 지원 범위는 Inspect가 실제 카메라에 적용해 검증합니다. 여러 카메라의 순차 검증을 위해 카메라 설정 요청은 최대 5분 기다립니다.
...
**같은 클래스에 여러 기준 이미지 등록:** 클래스별 **1~16장**, 이미지마다 박스 **1~64개**를 지원합니다.

1. 카메라·클래스를 선택하고 **새 이미지 촬영** 후 박스를 지정합니다.
2. **개정본 저장**을 누르면 해당 클래스의 기존 이미지에 새 이미지가 추가됩니다.
3. **현재 기준 이미지**의 이미지 번호를 누르면 기존 박스를 수정할 수 있습니다. 저장해도 같은 이미지가 중복 추가되지 않습니다.
4. **개정본에서 이미지 제외** 버튼은 그 이미지를 뺀 새 개정본을 저장합니다. 마지막 1장은 제외할 수 없습니다.
5. 사용할 개정본을 선택하고 **모델 빌드·검증 → 적용** 후 실제 검사 이미지로 확인합니다.

이미지 번호는 선택한 개정본의 목록 순서입니다. 이전 개정본 전체를 자동 누적하지 않고 선택한 개정본의 클래스 이미지 목록을 사용합니다.
기존 단일 이미지 개정본과 과거 모델·원본은 보존합니다. 서버는 모든 이미지의 특징을 클래스별로 같은 비중으로 합칩니다.
이미지 추가 자체는 재학습이나 운영 모델 적용이 아닙니다. 복수 기준 이미지를 지원하는 Inspect와 함께 배포해야 합니다.
