...
내부 메뉴는 **제품 레시피 / 생산 검사 / PLC 통신 진단 / PLC 디버그**입니다.
다른 메인 탭으로 이동해도 저장하지 않은 레시피 편집 내용과 내부 메뉴 선택은 유지됩니다.
...
**선택 신호 추가**에서 제품/포인트와 번호를 지정하고 신호 위치를 입력합니다. 기존 위치는 자동 이동하지 않습니다.
예: product_5=4, point_2=6, hardware_1=22를 유지하고 새 product_6=25를 추가할 수 있습니다.
하드웨어 4개 신호는 고정 항목이며 위치를 설정합니다. RX는 6~512칸, TX는 3~512칸입니다.
제품 카탈로그 추가와 PLC 매핑 추가는 별도입니다. 기본 제품은 5종이고 필요한 포인트 신호는 직접 등록합니다.
...
부위 버튼과 `검출 결과` / `원본`으로 저장된 PNG를 비교하고 휠·+/− 확대와 드래그 이동을 지원합니다.
...
- 마우스 휠·트랙패드 스크롤로 포인터 위치를 중심으로 확대하고, 오른쪽 아래 `−` / `+` 버튼으로도 조절합니다. 배율 버튼을 누르면 화면 맞춤으로 돌아갑니다. 화면 맞춤 기준 1~16배입니다.
...
- Images의 저장 이미지와 3D 포인트클라우드는 기존 전용 확대·축소 조작을 사용합니다.
